...
from house_photo_mapper.domain.services.plan_snippet import extract_plan_snippet
...
class ReportGeneratorService:
    ...
    def _render_page(
        ...
        try:
            plan_bytes = extract_plan_snippet(
                pdf_path=page_data.plan_pdf_path,
                page_index=page_data.plan_page_index,
                center_x=page_data.plan_center_x,
                center_y=page_data.plan_center_y,
                radius_meters=5.0,
                pixels_per_meter=page_data.plan_pixels_per_meter,
                target_width_px=int(plan_width),
                target_height_px=int(plan_height),
            )
            plan_img = ImageReader(io.BytesIO(plan_bytes))
            c.drawImage(
                plan_img,
                margin,
                plan_top - plan_height,
                width=plan_width,
                height=plan_height,
                preserveAspectRatio=True,
                anchor="nw",
            ...
            # Draw camera overlay on the plan snippet
            # Convert scene coordinates to local plan snippet coordinates
            plan_snippet_center_x = plan_width / 2
            plan_snippet_center_y = plan_height / 2
            cone_length = 40.0

            CameraOverlay.draw_camera_overlay(
                c=c,
                center_x=margin + plan_snippet_center_x,
                center_y=plan_top - plan_snippet_center_y,
                direction_angle=page_data.direction_angle,
                cone_angle=page_data.cone_angle,
                color=page_data.color,
                marker_radius=6,
                cone_length=cone_length,
            )
        except Exception:
            # If plan snippet fails, draw a placeholder
            c.setStrokeColorRGB(0.7, 0.7, 0.7)
